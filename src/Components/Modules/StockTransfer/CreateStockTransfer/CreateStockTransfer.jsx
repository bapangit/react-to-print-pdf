...
import BizClass from "./CreateStockTransfer.module.scss";
...
function CreateStockTransfer(props) {
  ...
  const getInvoiceCategoryOptions = async () => {
    ...
    if (response.ResponseCode === 1) {
      let filterInvoiceCategoryList = response.ResponseData.filter((data) => {
        return data.InvoiceCategoryID === 1003;
      });
  ...
  const updateState = (name, value) => {
    if (name === "selectedBranch") {
      ...
      formValues["selectedBranchShipTo"] = null;
      formValues["selectedBranchAddressShipTo"] = null;
      getBranchShipToOptions(value.CompanyAddressID);
    }
            ...
            <div className={BizClass.PreviewDiv_L1}>
              ...
              <div className={BizClass.PreviewDiv_L1_Box1}>
                <div className={BizClass.PreviewDiv_ShipTo}>
                  <Form.InputGroup label="Ship To">
                    <Form.InputControl
                      control="select"
                      options={branchShipToOptions}
                      isLoading={isLoadingBranchShipTo}
                      getOptionValue={(option) => `${option}`}
                      getOptionLabel={(option) => `${option.AddressDisplayName}`}
